...
function GastosPageInner({ tipoInicial, deInicial, ateInicial }: { tipoInicial: 'todos' | 'debito' | 'credito'; deInicial?: string; ateInicial?: string }) {
  ...
  const { totalDespesasFiltradas, saldoFiltrado } = useMemo(() => {
    ...
    return { totalDespesasFiltradas: desp, saldoFiltrado: rec - desp }
  }, [filtradas])
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 4, marginBottom: '1rem', flexWrap: 'wrap' }}>
              {(['categoria', 'evolucao', 'comparativo'] as const).map(a => (
                <button key={a} onClick={() => setAbaGrafico(a)} style={{
                  padding: '9px 14px', minHeight: 44, borderRadius: 6, border: 'none', cursor: 'pointer', fontSize: 12, fontWeight: 500,
            ...
            {abaGrafico === 'categoria' && (
              <div>
                {porCategoria.length === 0 ? (
                  ...
                  <div key={cat} role="button" tabIndex={0} style={{ marginBottom: 10, cursor: 'pointer' }}
                    onClick={() => setCatDrilldown(catDrilldown === cat ? null : cat)}
                    onKeyDown={e => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); setCatDrilldown(catDrilldown === cat ? null : cat) } }}>
