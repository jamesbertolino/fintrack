...
function useIsMobile() {
  ...
  if (typeof window !== 'undefined') {
    // leitura síncrona no primeiro render para evitar flash
  }
...
function LoginContent() {
    ...
    <div style={{ minHeight: '100vh', background: '#0a0a0a', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'system-ui', padding: '1.5rem' }}>
      <div style={{ maxWidth: 420, width: '100%', textAlign: 'center' }}>
        ...
        <p style={{ fontSize: 14, color: 'rgba(255,255,255,.5)', marginBottom: 24, lineHeight: 1.6 }}>
          Enviamos um link de confirmação para <strong style={{ color: '#4ade80' }}>{emailDigitado}</strong>.<br />
  ...
  // ── Layout mobile: coluna única com hero compacto no topo ──
  if (isMobile) return (
    <div style={{ minHeight: '100vh', background: '#0a0a0a', fontFamily: 'system-ui, sans-serif', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ flex: 1, padding: '1.5rem', overflowY: 'auto' }}>
        ...
        {/* Tabs */}
        <div style={{ display: 'flex', background: 'rgba(0,0,0,.3)', border: '1px solid #1a3a1a', borderRadius: 8, padding: 3, marginBottom: '1.25rem' }}>
        ...
        {tab === 'login' && (
          <form onSubmit={handleLogin}>
            <div style={{ fontSize: 18, fontWeight: 500, color: '#fff', marginBottom: 3 }}>Bem-vindo de volta</div>
            <div style={{ fontSize: 13, color: 'rgba(255,255,255,.4)', marginBottom: '1.25rem' }}>Continue subindo de nível</div>
            <GoogleBtn />
            <Divider />
            {[
            ...
            <button type="submit" disabled={loading} style={{
              width: '100%', padding: 13, background: '#16a34a', color: '#fff', border: 'none',
              borderRadius: 8, fontSize: 14, fontWeight: 500, cursor: loading ? 'default' : 'pointer', opacity: loading ? 0.6 : 1,
            }}>
        ...
        {tab === 'cadastro' && estado === 'form' && (
          <form onSubmit={handleCadastro}>
            <div style={{ fontSize: 18, fontWeight: 500, color: '#fff', marginBottom: 3 }}>Criar sua conta</div>
            <div style={{ fontSize: 13, color: 'rgba(255,255,255,.4)', marginBottom: '1.25rem' }}>7 dias grátis no plano Pro</div>
            <GoogleBtn />
            <Divider />
            {[
    ...
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', minHeight: '100vh', fontFamily: 'system-ui, sans-serif' }}>
      ...
      <div style={{ background: '#0f1f0f', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '2rem', borderLeft: '1px solid #1a3a1a' }}>
        <div style={{ width: '100%', maxWidth: 320 }}>
          ...
          <div style={{ display: 'flex', background: 'rgba(0,0,0,.3)', border: '1px solid #1a3a1a', borderRadius: 8, padding: 3, marginBottom: '1.5rem' }}>
            {(['login', 'cadastro'] as const).map(t => (
              <button key={t} onClick={() => { setTab(t); setErro(''); setEstado('form') }} style={{
          ...
          {erroParam === 'email_nao_confirmado' && (
            <div style={{ background: 'rgba(234,179,8,.1)', border: '1px solid rgba(234,179,8,.3)', borderRadius: 8, padding: '8px 12px', fontSize: 12, color: '#fde047', marginBottom: 12 }}>
              ⚠️ Confirme seu email antes de entrar. Verifique sua caixa de entrada.
            </div>
          )}
          {erro && (
            <div style={{ background: 'rgba(239,68,68,.1)', border: '1px solid rgba(239,68,68,.3)', borderRadius: 8, padding: '8px 12px', fontSize: 12, color: '#f87171', marginBottom: 12 }}>{erro}</div>
          )}

          {tab === 'login' && (
            <form onSubmit={handleLogin}>
              <div style={{ fontSize: 17, fontWeight: 500, color: '#fff', marginBottom: 3 }}>Bem-vindo de volta</div>
              <div style={{ fontSize: 12, color: 'rgba(255,255,255,.4)', marginBottom: '1.25rem' }}>Continue subindo de nível</div>
              <GoogleBtn />
              <Divider />
              {[
                { label: 'E-mail', key: 'email', type: 'email', placeholder: 'seu@email.com' },
                ...
                <div key={key} style={{ marginBottom: 12 }}>
                  <label style={{ display: 'block', fontSize: 10, fontWeight: 500, color: 'rgba(255,255,255,.5)', marginBottom: 5, textTransform: 'uppercase', letterSpacing: '.05em' }}>{label}</label>
                  <div style={{ position: 'relative' }}>
                    <input type={type} placeholder={placeholder}
                      value={loginForm[key as keyof typeof loginForm]}
                      onChange={e => setLoginForm(prev => ({ ...prev, [key]: e.target.value }))}
                    ...
                    {key === 'senha' && (
                      <button type="button" onClick={() => setShowPw(!showPw)}
                        style={{ position: 'absolute', right: 10, top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', cursor: 'pointer', color: showPw ? '#4ade80' : 'rgba(255,255,255,.3)' }}>
              ...
              <div style={{ textAlign: 'right', marginBottom: 12 }}>
                <span style={{ fontSize: 11, color: 'rgba(74,222,128,.7)', cursor: 'pointer' }}>Esqueci minha senha</span>
              </div>
              <button type="submit" disabled={loading} style={{
                width: '100%', padding: 11, background: '#16a34a', color: '#fff', border: 'none',
                borderRadius: 8, fontSize: 13, fontWeight: 500, cursor: loading ? 'default' : 'pointer', opacity: loading ? 0.6 : 1,
              }}>
                {loading ? 'Entrando...' : 'Entrar'}
              </button>
            </form>
          ...
          {tab === 'cadastro' && estado === 'form' && (
            <form onSubmit={handleCadastro}>
              ...
              <div style={{ fontSize: 12, color: 'rgba(255,255,255,.4)', marginBottom: '1.25rem' }}>7 dias grátis no plano Pro</div>
              <GoogleBtn />
              <Divider />
              {[
                { label: 'Nome',   key: 'nome',  type: 'text',  placeholder: 'Seu nome' },
              ...
              <div style={{ marginBottom: 12 }}>
                <label style={{ display: 'block', fontSize: 10, fontWeight: 500, color: 'rgba(255,255,255,.5)', marginBottom: 5, textTransform: 'uppercase', letterSpacing: '.05em' }}>Senha</label>
                <div style={{ position: 'relative' }}>
                  <input type={showPw ? 'text' : 'password'} placeholder="mín. 8 caracteres"
                    value={cadForm.senha}
                    onChange={e => { setCadForm(prev => ({ ...prev, senha: e.target.value })); calcularForca(e.target.value) }}
                ...
                {cadForm.senha && (
                  <>
                    <div style={{ height: 3, borderRadius: 2, marginTop: 5, background: 'rgba(255,255,255,.08)', overflow: 'hidden' }}>
                      <div style={{ height: '100%', borderRadius: 2, width: `${forca.pct}%`, background: forca.cor, transition: 'all .3s' }} />
                    </div>
              ...
              <div style={{ fontSize: 10, color: 'rgba(255,255,255,.2)', textAlign: 'center', marginTop: '.875rem', lineHeight: 1.6 }}>
                Ao criar sua conta você concorda com os{' '}
                <span style={{ color: 'rgba(74,222,128,.6)', cursor: 'pointer' }}>Termos</span> e a{' '}
                <span style={{ color: 'rgba(74,222,128,.6)', cursor: 'pointer' }}>Privacidade</span>
              </div>
          ...
          {tab === 'cadastro' && estado === 'sucesso' && (
            <div style={{ textAlign: 'center', padding: '.5rem 0' }}>
              ...
              <div style={{ display: 'inline-flex', alignItems: 'center', gap: 5, background: 'rgba(74,222,128,.1)', border: '1px solid rgba(74,222,128,.3)', borderRadius: 20, padding: '4px 12px', fontSize: 11, color: '#4ade80', fontWeight: 500, marginBottom: '.875rem' }}>
                +50 XP — conta criada!
              </div>
              <div style={{ fontSize: 16, fontWeight: 500, color: '#fff', marginBottom: 6 }}>Bem-vindo, {nomeUsuario}!</div>
              <div style={{ fontSize: 12, color: 'rgba(255,255,255,.5)', marginBottom: '1.25rem', lineHeight: 1.6 }}>
                Você está no nível 1. Verifique seu e-mail para ativar a conta e começar a subir de nível.
              </div>
              <button onClick={() => router.push('/dashboard')} style={{ width: '100%', padding: 11, background: '#16a34a', color: '#fff', border: 'none', borderRadius: 8, fontSize: 13, fontWeight: 500, cursor: 'pointer' }}>
                Ir para o app →
              </button>
            </div>
          )}
